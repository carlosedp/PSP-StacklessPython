...
#ifdef PSP
    // PSP note: sceKernelLibcGettimeofday() returns the time elapsed,
    // not since the Epoc, but since 00:00... We still use it to get a
    // better resolution.
...
#elif defined(PSP)
	Py_BEGIN_ALLOW_THREADS
        sceKernelDelayThreadCB((int)(secs * 1e6));
	Py_END_ALLOW_THREADS
#else
	/* XXX Can't interrupt this sleep */
	Py_BEGIN_ALLOW_THREADS
